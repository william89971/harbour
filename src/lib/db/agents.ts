...
export function createAgent(name: string, description?: string) {
  ...
  db.prepare(
    `INSERT INTO agents (id, name, description, api_key_hash) VALUES (?, ?, ?, ?)`
  ).run(id, name, description || null, apiKeyHash);
  return { id, name, description, apiKey };
}
...
export function getAgentById(id: string) {
  const db = getDb();
  return db.prepare(`SELECT id, name, description, last_polled_at, created_at, updated_at FROM agents WHERE id = ?`).get(id) as any || null;
}

export function listAgents() {
  const db = getDb();
  return db.prepare(`
    SELECT a.id, a.name, a.description, a.last_polled_at, a.created_at,
...
export function updateAgent(id: string, data: { name?: string; description?: string }) {
  const db = getDb();
  const fields: string[] = [];
  const values: any[] = [];
